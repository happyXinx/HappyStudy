...
![image-20210722162021242](D:\HappyStudy\Java学习\Java.assets\image-20210722162021242.png)



#### 2. 理解main方法的语法

* 由于java虚拟机需要调用类的main方法，所有该方法的访问权限必须是public，又因为Java虚拟机在执行main()方法时不必创建对象，所以该方法必须是static的，该方法接收一个String类型的数组参数，该数组中保存执行java命令时传递给所运行时的类和参数。
* 又因为main方法是静态的，我们不能直接访问该类中的非静态成员，必须创建该类的一个实例对象后，才能通过这个对象去访问类中的非静态成员。

#### 3. 代码块

代码块的作用： 对java类或对象进行初始化

一个列中代码块若有修饰符，则只能被static修饰，称为静态代码块。

|            | 静态代码块                       | 非静态                                             |
| ---------- | -------------------------------- | -------------------------------------------------- |
| 熟悉初始化 | 非静态不行                       | 都可以                                             |
| 执行顺序   | 按照从上到下的顺序执行           | 按照从上到下的顺序执行                             |
| 优先级     | 静态代码块执行优先于非静态       |                                                    |
| 加载       | 随着类的加载而加载，且只执行一次 | 每次创建对象的时候都会执行一次，且先于构造器执行。 |
...
再java中声明类、变量和方法时，可以使用关键字final来修饰。

final标记的类不能被继承。

​	String 、System 、StringBuffer类
...
![image-20210722170936739](D:\HappyStudy\Java学习\Java.assets\image-20210722170936739.png)
...
* 所有的成员变量默认由public static final修饰的；
* 所有的方法默认都是由public abstract修饰的；
* 接口中没有构造器
* 采用多继承机制
...
代码中有很多问题不是靠代码解决的，比如客户输入数据格式问题、网络不通畅等。

#### 1 异常概述

Java程序在执行过程中所发生的异常事件分为两类：

Error: Java虚拟机无法解决的严重问题。比如：StackOverflow和OOM。

Exception：其他因编程错误导致的一般性问题。
...
进程是程序的一次执行过程，或是正在运行的一个程序。进程是资源分配的基本单位
...
一个Java程序至少有三个线程：main() 、gc()垃圾回收线程、异常处理线程。
...
* 然后通过Thread对象的start（）方法来启动这个线程，而非直接调用run（）



JDK1.5之前创建新执行线程的两种方法：
...
* yield() 线程让步。让给优先级相同或者更高的线程去执行，若队列中没有相同优先级的线程，则忽略此方法。
* join()：当某个程序调用其他线程的join()方法时，调用线程将被阻塞，直到join()方法加入的join线程执行完为止。
* sleep(): 令当前活动线程在指定时间段内放弃对CPU的控制
...
守护线程是用来服务用户线程的，通过start()方法前调用thread.setDaemon(true)可以把一个用户线程变成一个守护进程。
...
![image-20210729235431672](D:\HappyStudy\Java学习\Java.assets\image-20210729235431672.png)
